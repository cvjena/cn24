...
MultiClassErrorLayer::MultiClassErrorLayer(const unsigned int classes)
: classes_(classes) {
...
bool MultiClassErrorLayer::CreateOutputs (const std::vector< CombinedTensor* >& inputs,
...
bool MultiClassErrorLayer::Connect (const std::vector< CombinedTensor* >& inputs,
...
void MultiClassErrorLayer::FeedForward() {
...
#pragma omp parallel for default(shared)
  for (std::size_t i = 0; i < first_->data.samples(); i++) {
    ...
    for(unsigned int c = 0; c < classes_; c++) {
      const datum diff = *(first_->data.data_ptr_const(c, 0, 0, i))
...
void MultiClassErrorLayer::BackPropagate() {
  // The deltas are already written in to the input CombinedTensors, so
  // there is nothing to do now.
}

datum MultiClassErrorLayer::CalculateLossFunction() {
  datum error = 0;
